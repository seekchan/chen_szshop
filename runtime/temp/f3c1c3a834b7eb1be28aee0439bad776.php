<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:58:"D:\Szshop\public/../application/admin\view\index\menu.html";i:1532694366;}*/ ?>
...
.explode {
  background:url(__ADMIN__/Images/menu_minus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.collapse {
  background:url(__ADMIN__/Images/menu_plus.gif) no-repeat 0px 3px;
  font-weight:bold;
}
.menu-item {
  background:url(__ADMIN__/Images/menu_arrow.gif) no-repeat 0px 3px;
  font-weight:normal;
}
#help-title {
  font-size:14px;
    ...
    <p>
        <span style="float:right; padding:3px 5px;" >
            <a href="javascript:toggleCollapse();">
                <img id="toggleImg" src="__ADMIN__/Images/menu_minus.gif" width="9" height="9" border="0" alt="闭合" />
            </a>
        </span>
        ...
        <?php if(is_array($menus) || $menus instanceof \think\Collection || $menus instanceof \think\Paginator): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;if($vo['parent_id'] == '0'): ?>
            ...
            <?php echo $vo['privilege_name']; ?>
                ...
                <?php if(is_array($menus) || $menus instanceof \think\Collection || $menus instanceof \think\Paginator): $i = 0; $__LIST__ = $menus;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;if($v['parent_id'] == $vo['id']): ?>
                    <li class="menu-item"><a href="<?php echo url($v['controller_name'].'/'.$v['action_name']); ?>" target="main-frame"><?php echo $v['privilege_name']; ?></a></li>
...
<script type="text/javascript" src="__ADMIN__/Js/utils.js"></script>
...
<script language="JavaScript">
    ...
    function toggleCollapse() {
        ...
        collapse = !collapse;
        document.getElementById('toggleImg').src = collapse ? '__ADMIN__/Images/menu_minus.gif' : '__ADMIN__/Images/menu_plus.gif';
